      ...
      {menuOpen && (
        <div className="border-t border-brand-primary/30 bg-brand-deep px-6 pb-6 lg:hidden" style={{ transition: 'var(--transition-drawer)' }}>
          <div className="flex flex-col gap-1 py-4">
            <Link href="/" className={`rounded-sm px-4 py-3 font-medium hover:bg-brand-primary/20 ${pathname === '/' ? 'text-brand-accent' : ''}`} onClick={() => setMenuOpen(false)}>Home</Link>
            <div className={`px-4 py-2 text-sm text-brand-sky/70 ${pathname.startsWith('/products') ? 'text-brand-accent font-medium' : ''}`}>Products</div>
            {categories.map((cat) => (
              <Link key={cat.id} href={`/products/${cat.slug}`} className={`rounded-sm px-8 py-2 hover:bg-brand-primary/20 ${pathname === `/products/${cat.slug}` ? 'text-brand-accent bg-brand-primary/10' : ''}`} onClick={() => setMenuOpen(false)}>
                {cat.name}
              </Link>
            ))}
            <Link href="/about" className={`rounded-sm px-4 py-3 font-medium hover:bg-brand-primary/20 ${pathname === '/about' ? 'text-brand-accent' : ''}`} onClick={() => setMenuOpen(false)}>About</Link>
            <Link href="/contact" className={`rounded-sm px-4 py-3 font-medium hover:bg-brand-primary/20 ${pathname === '/contact' ? 'text-brand-accent' : ''}`} onClick={() => setMenuOpen(false)}>Contact</Link>
          </div>
